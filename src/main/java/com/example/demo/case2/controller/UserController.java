...
public class UserController {
	 ...
	 * If successful, generate a JWT token consists of the authenticate details
	 * A JSON object is created consists of token,account_status,role
	 */
	 ...
	 * Call user service to make the registration by passing user details
	 */
	...
	/*
	 * Card Application
	 * Apply for a specific credit card type
	 * Get JWT token from request and grab email id
	 */
	...
	public ResponseEntity<?> getCardStatus(HttpServletRequest request) throws Exception{
		...
		AuthTokenFilter ATF = new AuthTokenFilter();
		List<JSONObject> list = refUserService.getCardApplicationStatus(jwtUtils.getUserNameFromJwtToken(ATF.parseJwt(request)));
		if(!list.isEmpty()) {
			return ResponseEntity.ok().body(list.toString());
		}
		else {
	...
	public ResponseEntity<?> getPaymentDetails(HttpServletRequest request) {
		AuthTokenFilter ATF = new AuthTokenFilter();
		List<JSONObject> list = refUserService.getPaymentHistory(jwtUtils.getUserNameFromJwtToken(ATF.parseJwt(request)));
		return ResponseEntity.ok().body(list.toString());
	}
